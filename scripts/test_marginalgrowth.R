...
caribou_datat <- matrix(0,nrow = length(cA3xyzl$cariboucA3), ncol = 6)
#simulate at nodes
for(j in 1:NumNodes){
  caribou_datat[j,1]<-cA3xyzl[j,1]
  caribou_datat[j,2]<-cA3xyzl[j,2]
  caribou_datat[j,3]<-cA3xyzl[j,3]
  caribou_datat[j,4]<-cA3xyzl[j,4]
  caribou_datat[j,5]<-xdot(cA3xyzl[j,1], cA3xyzl[j,2], cA3xyzl[j,3], cA3xyzl[j,4], sim.parmsE)
  caribou_datat[j,6]<-wval(cA3xyzl[j,1], cA3xyzl[j,2], cA3xyzl[j,3], cA3xyzl[j,4], sim.parmsW)
...
for(j in 1:NumNodes){
  ...
  caribou_datat[j,5]<-xdot(cF3xyzl[j,1], cF3xyzl[j,2], cF3xyzl[j,3], cF3xyzl[j,4], sim.parmsE)
  caribou_datat[j,6]<-wval(cF3xyzl[j,1], cF3xyzl[j,2], cF3xyzl[j,3], cF3xyzl[j,4], sim.parmsW)
  #caribou_data1[j,6]<-ydot(xs[j], ys[j], zs[j], ls[j], sim.parmsE)
  #caribou_data1[j,7]<-zdot(xs[j], ys[j], zs[j], ls[j], sim.parmsE)
 # caribou_data1[j,8]<-ldot(xs[j], ys[j], zs[j], ls[j], sim.parmsE)
 # caribou_data1[j,9]<-wval(xs[j], ys[j], zs[j], ls[j], sim.parmsW)
...
colnames(caribou_datat) <- c("caribou","prey","wolves","linearfeature", 
                            "caribougrowth", "wval")


caribou_dataF3 <- as.data.frame(caribou_datat)  #87, .25
caribou_dataA3 <- as.data.frame(caribou_datat)  #70, .25
